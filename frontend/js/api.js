...
// ── Ouvrir un PDF avec token dans l'URL ──────
function ouvrirPDF(numeroRecu) {
  const token = Auth.getToken();
  if (!token || !numeroRecu) return;
  const url = `${API_BASE}/recus/${numeroRecu}/pdf?token=${encodeURIComponent(token)}`;
