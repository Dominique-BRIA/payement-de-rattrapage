...
// ─────────────────────────────────────────────────────────
// NOTCHPAY — Vérifier la signature HMAC du webhook
// ─────────────────────────────────────────────────────────
function verifierSignatureNotchPay(body, signatureRecue) {
  if (!process.env.NOTCHPAY_HASH) return true; // pas de hash configuré → on laisse passer
  const expectedHash = crypto
    .createHmac('sha256', process.env.NOTCHPAY_HASH)
    .update(JSON.stringify(body))
    .digest('hex');
  return signatureRecue === expectedHash;
}
...
router.post('/mobile', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // ✅ Le frontend redirige l'étudiant vers authorization_url
    res.status(201).json({
      success: true,
      ...
      data: {
        ...
        transaction_id,
        montant_total,
        authorization_url,  // ← frontend redirige ici
...
// Appelé par le frontend quand l'étudiant revient après paiement NotchPay
// ─────────────────────────────────────────────────────────
...
router.post('/webhook', express.json(), async (req, res) => {
  ...
  // ✅ Vérification HMAC correcte
  const signatureRecue = req.headers['x-notch-signature'];
  if (!verifierSignatureNotchPay(event, signatureRecue)) {
