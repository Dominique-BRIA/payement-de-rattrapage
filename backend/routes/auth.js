  ...
  try {
    const result = await db.query(
      `SELECT e.*, f.nom AS filiere_nom, f.code AS filiere_code
       ...
       WHERE e.matricule = $1 AND e.telephone = $2`,
      [matricule, telephone]
    );
    ...
    res.json({
      ...
      etudiant: {
        id: etudiant.id, matricule: etudiant.matricule,
        nom: etudiant.nom, prenom: etudiant.prenom,
        email: etudiant.email, telephone: etudiant.telephone,
